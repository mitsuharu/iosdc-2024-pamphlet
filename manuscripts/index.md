...
d は画像データです。1bit ごとに色情報（0 or 1）を表現する 1bit Bitmap です。たとえば、n 番目の色情報を `c(n)` とすると、i 番目の d は次のように設定します（i は 0 ~ (width * height)/8 の自然数）。なお、幅サイズが 8 の倍数でなければ 0 を埋めておきます。
