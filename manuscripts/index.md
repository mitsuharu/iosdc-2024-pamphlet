...
我々 iOS アプリエンジニアは（バージョンで差異はありますが）１つの Swift で開発しているので、環境それぞれでコマンドが異なるのは衝撃的です。それを知ると、バイトコードは低レイヤーで複雑だし書きづらい、ESC/POS コマンドは書きたくない！と手のひらを返します。しかし、捨てる神あれば拾う神あり、レシート印刷に便利な OSS が存在します。
